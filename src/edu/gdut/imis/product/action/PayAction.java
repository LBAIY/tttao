...
//	Users user=(Users) context.getSession().get("user");	
	...
	public String createOrder() {
		ActionContext context=ActionContext.getContext();
		Users user=(Users) context.getSession().get("user");
		System.out.println("1yonghu"+user.getuID());
		...
		List<Cartproduct> list=payEbi.findAll(user.getuID());
		...
		for(int i=0;i<codes.length;i++) {
			for(Cartproduct cp:list){
				//System.out.println("2gouwuche"+cp.getCode());
...
//		System.out.println("用户"+user.getuID()+"  订单编号："+OrderNumber);
...
//			op.setCode(code);
//			op.setCount(count);
//			System.out.println(op.getCode()+"  数量："+op.getCount());
//		}
